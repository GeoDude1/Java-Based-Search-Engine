...
public class TextFileStemmer {
	...
	/**
	 * This a function that initializes the ArrayList that will be parsed.
	 * Created so it would not be called multiple times.
	 * 
	 * @param line the line of words to clean, split, and stem
	 * @param stemmer the stemmer to use
	...
	public static void stemLine(String line, Stemmer stemmer, Collection<String> stems) {
		...
		for(int i = 0 ; i < list.length; i++) {
			stems.add(stemmer.stem(list[i]).toString());
		}
	}
	...
	public static ArrayList<String> listStems(Path inputFile) throws IOException {
		...
		try(BufferedReader read = Files.newBufferedReader(inputFile);){
			String line = null;
